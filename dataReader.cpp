...
bool dataReader::loadOneRowDataFile( const char* filename, int nI, int nT )
{
    //clear any previous data
    for (int i=0; i < (int) data.size(); i++ ) delete data[i];
    data.clear();
...
for ( int i = 0; i < 1; i++ ) tSet.trainingSet.push_back( data[i] );
        //generalization set
        for ( int i = 0; i < 1; i++ ) tSet.generalizationSet.push_back( data[i] );

        //validation set
        for ( int i = 0; i < 1; i++ ) tSet.validationSet.push_back( data[i] );
